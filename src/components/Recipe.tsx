...
const CodePreview: FC<CodePreviewProps> = ({ filename, sourceUrl, children }) => (
  <motion.div 
    ...
    transition={{ duration: 0.3 }}
  >
    <div className="rounded-lg overflow-hidden border border-gray-200 transition-all duration-200 group-hover:opacity-50 group-hover:border-gray-300">
    ...
    >
      <span className="px-4 py-2 bg-[#565872] text-white rounded-lg flex items-center gap-2 hover:bg-[#6E7087] shadow-lg transition-colors duration-200">
    ...
    <AnimatePresence mode="wait">
      {!isExpanded ? (
        ...
        >
          ...
          >
            <span className="px-4 py-2 bg-white text-gray-900 rounded-lg shadow-lg flex items-center gap-2 hover:bg-gray-50 hover:scale-105 transition-all duration-200">
        ...
        <motion.div 
          className="mt-4 mb-4 bg-white rounded-xl overflow-hidden border border-gray-200"
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: 'auto' }}
          exit={{ opacity: 0, height: 0 }}
          transition={{ duration: 0.3 }}
            ...
            <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
              ...
              <div className="flex flex-col xl:flex-row gap-3 w-full xl:w-auto">
                <a 
                  href={github}
                  target="_blank"
                  className="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-gray-200 hover:border-gray-300 hover:bg-gray-50 hover:scale-105 transition-all duration-200"
                >
                ...
                {demo && (
                  <a 
                    href={demo}
                    target="_blank"
                    className="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-[#565872] text-white hover:bg-[#6E7087] hover:scale-105 transition-all duration-200"
                  >
                    Try Demo →
                  </a>
                )}
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-4">
                {tags.map(tag => (
                  <span 
                    key={tag} 
                    className="px-2.5 py-1 text-sm bg-[#F7F7F8] text-[#565872] rounded-md border border-[#E5E5E7] transition-colors duration-200 hover:bg-[#EDEDF0]"
                  >
                    {tag}
                  </span>
                ))}
