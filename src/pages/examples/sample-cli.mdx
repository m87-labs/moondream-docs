...
This example demonstrates how to use Moondream's CLI interface for image analysis and interactive questioning.
...
Save this code as `sample.py` and run it using one of the following commands:

1. Single question mode:
```bash
python sample.py --image [IMAGE_PATH] --prompt "What's in this image?"
```
...
3. Caption mode (generate image caption):
```bash
python sample.py --image [IMAGE_PATH] --caption
```

4. CPU mode (if you encounter GPU issues):
```bash
python sample.py --image [IMAGE_PATH] --cpu
```
...
- Support for both single questions and interactive mode
